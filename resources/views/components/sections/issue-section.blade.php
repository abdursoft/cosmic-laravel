<section class="w-full bg-[#1D0501] text-white min-h-[77vh] py-20 px-4 md:px-45">
    ...
    <div class="flex mt-5 w-full flex-col md:flex-row flex-wrap">
        @foreach($issues as $key=>$issue)
            <div class="w-full h-[330px] @if($key < 3) md:w-1/3 md:h-[469px] @else md:w-1/2 md:h-[700px] @endif p-3">
                <div class="w-full h-full rounded-[22px] shadow-md h-md bg-white relative">
                    ...
                    <div class="w-full h-full absolute rounded-md top-0 left-0 items-center justify-end flex-col {{$issue->issue_type == 'free' ? 'flex' : 'hidden'}}">
                        <a href="{{route('issue.read',$issue->id)}}" class="bg-gray-600 rounded-lg cursor-pointer mb-4 px-3 py-2">Read Issue</a>
                    </div>
                </div>
            </div>
        @endforeach
