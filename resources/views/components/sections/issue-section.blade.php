<section class="w-full bg-[#1D0501] text-white min-h-[77vh] py-20 px-4 md:px-5 lg:px-45">
        ...
        @foreach($issues as $key=>$issue)
            <div class="w-full h-[330px] @if($key < 3) md:w-1/3 md:h-[469px] @else md:w-1/2 md:h-[700px] @endif p-3" data-aos="zoom-in">
                <div class="w-full h-full rounded-[22px] shadow-md h-md bg-white relative">
                    ...
                    <div class="w-full h-full absolute rounded-md top-0 left-0 items-center flex justify-center flex-col group hover:bg-[rgba(105,192,140,0.5)]">
                        <a href="{{route('issue.read',$issue->id)}}" class="bg-gray-600 md:hidden text-white rounded-lg cursor-pointer mb-4 px-3 py-2 group-hover:flex">Read Issue</a>
                    </div>
                </div>
            </div>
        @endforeach
