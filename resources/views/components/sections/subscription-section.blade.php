        ...
        @foreach($packages as $key=>$package)
            <div class="w-full h-full @if($key < 3) md:w-1/3 h-[469px] @else md:w-1/2 h-[700px] @endif" data-aos="zoom-in">
                <div class="w-full h-full h-md relative text-gray-800 p-3">
                    <img src="{{Storage::url($package->thumbnail)}}" loading="lazy" alt="" class="w-full bg-white h-[350px] md:h-[550px] rounded-[22px] object-cover">
                    ...
                    <div class="flex items-center justify-center">
                        <a href="{{ route('user.subscribe',$package->id) }}" class="text-center p-3 bg-gray-300 rounded-md mt-5 w-full">{{ $package->cta_text }}</a>
                    </div>
                </div>
            </div>
        @endforeach
...
<div class="w-full bg-white py-20 md:px-24">
    <div class="w-full flex flex-col items-center justify-center" data-aos="fade-up">
        ...
        <img src="{{asset('images/features.webp')}}" loading="lazy" class="w-full rounded-[10px] max-w-4xl h-[86vh]" alt="">
    </div>
</div>
