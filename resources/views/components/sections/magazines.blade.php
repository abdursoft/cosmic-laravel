    ...
    <div class="flex mt-5 w-full flex-col md:flex-row flex-wrap mb-8 h-auto">
        @foreach ($subscriptions as $subscription)
            @if ($subscription->package)
                @if ($subscription->package->magazines)
                    @foreach ($subscription->package->magazines as $magazine)
                        <div
                            class="text-gray-800 rounded-[22px] flex flex-col relative h-70 md:h-85 lg:h-95 w-full md:1/2 lg:1/3 xl:w-1/4 max-w-md shadow-md overflow-hidden">
